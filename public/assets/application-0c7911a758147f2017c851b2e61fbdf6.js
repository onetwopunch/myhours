...
(function() {
  $(function() {
    ...
    */
    ...
    var addErrorMessage, addErrorStyle, email, emailRegEx, email_tag, err_email, err_non_match, err_pass_len, err_user_exists, err_user_not_exists, f_email, inputs, pass_tag, password, passwordRegEx, passwords, removeErrorMessage, removeErrorStyle, rep_pass, rep_pass_tag, user_is_new, validateEmail, validateForm, validateMatchingPasswords, validatePassword, validateUserExists;
    ...
    */
    ...
    rep_pass.blur(function() {
      return validateMatchingPasswords(password, rep_pass);
    });
    /*
    ...
    */
    ...
    email = $('#user_email');
    email.blur(function() {
      return validateEmail(email, emailRegEx, false);
    });
    /*
    ...
    */
    ...
    f_email.blur(function() {
      return validateUserExists(f_email, true);
    });
    /*
    ...
    */
    ...
    $('#forgot-email-form').submit(function(event) {
    ...
    validateMatchingPasswords = function(p1, p2) {
      var passes;
      passes = true;
    ...
    validatePassword = function(password, isMatch) {
      var passes;
      passes = true;
    ...
    validateUserExists = function(email, we_want_user_to_exist) {
      var user_exists;
      user_exists = false;
      console.log("we_want_user_to_exist: " + we_want_user_to_exist);
      return $.ajax('/login/check_email_exists', {
        type: 'POST',
        data: {
          email: $(email).val()
        },
        ...
        error: function(jqXHR, textStatus, errorThrown) {
          return alert("<b>Oops!</b><br><p>There was a server error validating your email:</p>" + errorThrown);
        },
        success: function(data, textStatus, jqXHR) {
          console.log("Request successful");
          user_exists = data['response'] === "user_exists";
          return console.log("Request user_exists?: " + user_exists);
        },
        complete: function(jqXHR, textStatus) {
          ...
          if (we_want_user_to_exist) {
            ...
            if (user_exists) {
              ...
              user_is_new = false;
              addErrorStyle(email);
              return addErrorMessage(email_tag, err_user_exists);
            } else {
              console.log("Complete user_exists false");
              user_is_new = true;
              removeErrorStyle(email);
              return removeErrorMessage(email_tag, err_user_exists);
            }
          }
        }
      });
    ...
    */
    ...
    addErrorStyle = function(element) {
      return $(element).addClass('error-field');
    };
    removeErrorStyle = function(element) {
      return $(element).removeClass('error-field');
    };
    addErrorMessage = function(element, message) {
      return $(element).html(message);
    };
    return removeErrorMessage = function(element) {
      return $(element).html('');
    };
  });
...
(function() {
  $(function() {
    var post_to_create, post_to_remove, post_to_show;
    post_to_show = function(id) {
      return $.post('/profile/show', {
        entry_id: id
      }, function(data) {
        $('#desc-out').html(data['description']);
        $('#user-out').html(data['username']);
        return $('#pass-out').html(data['password']);
      });
    };
    post_to_remove = function(id) {
      return $.post('/profile/delete', {
        entry_id: id
      }, function(data) {
        return $(".entry.selected").remove();
      });
    };
    post_to_create = function(desc, un, pw) {
      return $.post('/profile/create', {
        description: desc,
        username: un,
        password: pw
      }, function(data) {
        return $(".entry-box ul").prepend("<li class='entry' id='" + data['entry_id'] + "'>" + data['description'] + "</li>");
      });
    };
    ...
    $('#modal-show').click(function() {
      ...
      return post_to_show(int_id);
    });
    $('#create-entry-submit').click(function() {
      var desc, pw, un;
      ...
      return post_to_create(desc, un, pw);
    });
    $('#show-entry').on('hidden.bs.modal', function() {
      $('#desc-out').html('');
      $('#user-out').html('');
      $('#pass-out').html('');
      return $.post('/profile/hide', function(data) {
        return console.log("password data removed from browser and server");
      });
    });
    return $('#remove-entry').click(function() {
      var int_id, str_id;
      str_id = $('.entry.selected').attr('id');
      int_id = parseInt(str_id, 10);
      return post_to_remove(int_id);
    });
  });
